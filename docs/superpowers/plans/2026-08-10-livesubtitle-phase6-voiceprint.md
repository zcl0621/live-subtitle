...
- [ ] 长会话(>30min):确认簇不爆炸(同一人被拆成多个说话人 = θ_cluster 偏高)、内存平稳
      **簇爆炸的补救已就位**:设置页两根阈值滑杆(运行中置灰),调低 θ_cluster 即可并簇
...
- [ ] **改名的跨会话陷阱**(已知限制,不阻塞):`speakerNames` 按 `SpeakerID.Kind` 键控、
      没有会话维度,而 `beginSession` 会清空它;若在新一场刚开始、还没出终句时
      去改屏上遗留的上一场行的名字,那个名字会落到**新一场的簇号命名空间**上。
      触发要件苛刻(停止→开始→未出终句→改名),真机若真遇到,再按
      `[sessionID: [Kind: String]]` 改键并去掉 `beginSession` 里的清空。
