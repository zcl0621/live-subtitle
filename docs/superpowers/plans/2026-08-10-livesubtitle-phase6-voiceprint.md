...
> **状态(2026-08-15 合入时)**:部分完成。已跑过 **YouTube 视频(双说话人)** 与 **本人麦克风** 两类场景,
> 并据此修掉 4 个真 bug(录制崩溃、判定下限、簇爆炸、导出丢改名)。
> **未跑真实会议**(用户当时没有会议可测),下面未打勾的项就是欠账,下次开会时补。
> 已确认可用的证据(诊断日志实测):模型 0.5–0.7s 就绪;本人正常说话 meScore 0.79–0.80 稳判「我」;
> 两个陌生说话人分开(簇内 0.71–0.86,簇间 −0.02);单次判定 50–65ms。

- [x] **麦克风轨认出「我」**:正常语气两句(9.96s / 9.12s)meScore 0.804 / 0.789,均判为「我」;
      其间两句 1.92s 的短句按设计回退沿用「我」
- [x] **系统音轨区分陌生说话人**:同人簇内余弦 0.708–0.857、异人 −0.023,分得开
...
- [ ] 长会话(>30min):确认簇不爆炸(同一人被拆成多个说话人)、内存平稳
      **补救已就位两层**:① 设置页三根滑杆(最短判定句长 / θ_me / θ_cluster,运行中置灰);
      ② 聚类的「死区」(RESULTS.md §Task 10 观测):簇分落在 [0.20, θ_cluster) 时沿用上一句身份、
      不新建簇 —— 2026-08-15 实测的簇爆炸正是被污染的单句造成的,不是阈值差一点
- [ ] 记录主观正确率,写回 `RESULTS.md`
- [x] **撤掉临时诊断日志**(已于合入前完成:删 Diagnostics.swift + 30 处调用点,纯删日志、逻辑未动)
      ~~原文~~:`Sources/LiveSubtitle/Diagnostics.swift` + 各处 `lslog(...)` 调用点
      (删掉那个文件,编译器会把所有调用点报出来)。它在 Task 10 期间是主力排障工具
      —— 「声纹没启用」「麦克风不识别」两次误判都是靠它一次定位的 —— 但**不能进主干**。

**终审(2026-08-15)转来的三条,验收时一并看:**

- [ ] **P6b 负载**(DEGRADE 闸门,一直没单独跑):双轨 + 双声纹链路跑满 10 分钟,
      记 CPU%/内存/机身温度/有无掉帧。plan 第 298 行「若 append 成为热点再换 memcpy」
      这个决定悬着,靠这次测量落地。
      **已有零头数据**(2026-08-15 诊断日志,非正式测量):模型加载 + 预热 0.5–0.7s;
      单次判定(切片 + Float 转换 + CoreML 推理)**50–65ms**。缺的是持续负载与热。
- [ ] **小窗焦点行为**(Task 8 引入 `KeyablePanel` 才能让改名输入框拿到键盘焦点):
      开会中拖一下字幕小窗,然后确认会议 app 的快捷键(推话/静音)仍然生效。
      若被吞:把 `canBecomeKey` 收窄成「正在改名时才为 true」。
- [x] ~~**改名的跨会话陷阱**~~ —— **已修**(2026-08-15 review 跟进,commit 92d2a49)。
      原问题:`beginSession` 当场清空 `speakerNames`,而导出锚在【最后一条终句】所在的那一场,
      于是「停止→重开→未出终句→导出」会拿空映射去导上一场的行,改的名全丢。
      改法:清空推迟到**本场第一条终句**(`renamesPendingExpiry`),与导出锚点切换用同一个时刻。
      验收时顺手复验一遍:改名 → 停止 → 重开 → 先别说话 → 导出,笔记里应该还是你改的名字。
