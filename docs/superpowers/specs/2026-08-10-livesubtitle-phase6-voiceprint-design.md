...
终句的 audioTimeRange
  ...
  → 与「我」的两份档案比 cosine,取 max
      ≥ θ_me(默认 0.60,P6c 实测定值) → 判定为「我」
  → 否则与本会话已有聚类中心比 cosine
      ≥ θ_cluster(默认 0.50,P6c 实测定值) → 归入该簇,更新簇中心(滑动平均后重新 L2 归一化)
      否则 → 新建簇「说话人 N」
```

> **θ 定值依据(P6c,2026-08-15,详见 probes/RESULTS.md;替代 P6a 的 0.70/0.60):**
> P6a 是拿「整段 vs 整段」标的(且当时未察觉 FluidAudio 内部只取前 10s),
> 而真实链路是**多窗平均的注册档案 × 会话里一条几秒的终句**。后者同人余弦系统性偏低:
> 5s 句 min 0.677、10s 句 min 0.710 —— θ_me=0.70 会把自己判成别人(**漏判「我」**,
> 不是误判)。异人 max 在 3/5/10s 三档都稳定在 0.40–0.42,故下调到 0.60/0.50
...
5s 句才回到 +0.272)的终句 embedding 不可靠,**沿用该轨上一句的身份**(连续性启发式),不新建簇。
