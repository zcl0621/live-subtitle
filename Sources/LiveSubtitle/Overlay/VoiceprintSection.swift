...
struct VoiceprintSection: View {
    ...
    var body: some View {
        Section("我的声纹") {
            ...
            // 两份档案【始终同时生效】,不按会议语种挑档案:匹配时两份都载入取 max
            // (spec §1,省掉「按语种选档案」的分支逻辑)。文案必须照实说 ——
            // 说成「分别在对应语种的会议里生效」,用户就会以为只录中文那份在英文会议里没用。
            Text("录一段自己的声音,字幕里就能把你说的话标成「我」。两份档案任何会议里都一起生效,中英各录一份只是让覆盖更全;录完下一场字幕开始时生效。")
                .font(.caption).foregroundStyle(.secondary)
