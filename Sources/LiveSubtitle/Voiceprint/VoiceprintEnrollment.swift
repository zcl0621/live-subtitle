...
/// 显式截前 160_000 样本),直接喂一段 25s 的录音等于把后 15s 扔掉 —— 那 P6a
/// 「档案要 ≥20s 语音」这条要求就白提了,用户读满整段文字也没换来更稳的档案。
/// 故切成模型窗口大小的片段逐段抽取,再取平均并重新 L2 归一化。
///
/// 平均后归一化是 `SpeakerClusterer.update(cluster:with:)` 更新簇心的同一套做法:
/// 同一个人的若干条已归一化向量取质心,仍落在同一向量空间,聚类阈值(θ_me=0.70)不受影响,
/// 且比任何单窗都更稳(单窗会被那 10s 里的语调/内容带偏)。
enum VoiceprintEnrollment {
    ...
    /// 模型窗口:10s @ 16kHz。与 `FluidAudioExtractor.embed` 的截断长度一致。
