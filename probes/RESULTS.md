...
## Task 10 真机观测(2026-08-15)—— 离线标定在真实视频音频上打折,加「死区」

把 minDuration 降到 2.0s 后重跑同一段 YouTube,判定覆盖率如期上升,但暴露两件事:

### 1. 簇爆炸的真实成因不是阈值差一点,是「某一句的向量坏了」

同一个说话人相邻两句:

```
11.64..20.40 (8.76s)「It's got a leak internal somewhere」→ 簇分 0.857 → 并入【说话人 2】✅
21.54..25.02 (3.48s)「Now, after seeing all that cool...」→ 簇分 0.248 → 新建【说话人 3】❌
```

0.248 不是「差一点没够着 0.50」,是差了一大截。要靠调 θ_cluster 救它得降到 0.24 ——
而 P6d 实测异人最高 0.404,降到那儿会开始把两个人并成一个(硬错)。**这条路走不通。**

线索:那一句结束于 25.02,下一条终句要到 31.50 —— 中间 6.5s 没有任何语音。多半是视频转场,
音乐盖着人声把 embedding 带偏了。这类污染在 P6d 的标定集(无噪朗读)里一条都没有。

**改法:新建簇的门槛与「没匹配上」分开。** 原逻辑只有两档(≥θ_cluster 并入,否则新建),
于是任何被污染的向量都会凭空造一个说话人。改成三档,新增 θ_new = 0.20:

| 最高簇分 | 处置 |
|---|---|
| ≥ θ_cluster (0.50) | 并入该簇(并更新簇心) |
| [0.20, 0.50) **死区** | 说不准 → 沿用上一句身份,**不新建簇、也不更新簇心** |
| < 0.20 | 确实是没见过的人 → 新建簇 |

拿本次三条判定回代:0.098 → 新建(对,确是新人)、**0.248 → 沿用【说话人 2】(修掉了报的 bug)**、
0.080 → 新建(对,那是买车那位)。θ_new=0.20 的依据:实测新人分数落在 0.08–0.14,
被污染的同人句落在 0.25 上下,0.20 正好切开;且与 θ_cluster 的可调下限 0.30 恒有余量。

代价是软错:新人的**第一句**若恰落在死区,会被算作上一个人,等他下一句清楚了才立簇。

### 2. ~~⚠️ 离线标定对真实条件偏乐观~~ —— **本节结论已被后续实测推翻,见下方「更正」**

```
1.32..3.96 (2.64s) [mic]「Hello, hello, hello, hello.」meScore = 0.409(θ_me=0.60)
→ 没认出是本人,标成了【说话人 1】
```

P6d 说同人 2.5s 档最低 0.574、中位 0.770;实测本人对自己的档案只有 **0.409** —— 已经低到
P6d 测出的**异人最高值(0.404)**附近。也就是说:**在真实条件下,同人与异人的分布是重叠的,
而离线语料上它们隔着 0.17 的间隔。**

原因(推测,未验证):标定集是同一次录音、同一麦克风距离、朗读语调;真实场景是短促口语、
距离语速语调都不同,再叠上视频侧的压制音频与背景音。

**所以别再拿 P6d 的绝对数字当真值上限。** 它能证明的只有相对结论(硬错在离线条件下为 0、
两条路径分布重合、句长越短越差),不能保证真实场景同样有余量。θ_me 目前维持 0.60 未动 ——
样本只有一句、且「Hello hello hello hello」这种重复短语本就不利于声纹,不够重定阈值。
要重定得先攒一批正常语气的真实样本。

### 更正(同日稍后,正常语气实测)—— 上一节的 θ_me 结论不成立

补测:对着麦克风用**正常语气说完整的话**,同一份档案:

```
9.96s「I receive a message from someone saying…」meScore = 0.804 → 判为【我】✅
1.92s「The price was so good.」                → 短于下限,回退沿用「我」 ✅
1.92s「I couldn't say no.」                    → 同上 ✅
9.12s「Only issue is I only have one week…」   → meScore = 0.789 → 判为【我】✅
```

**0.79–0.80,与 P6d 的离线分布(5.0s 档同人中位 0.862、最低 0.677)吻合。**
所以上一节那个 0.409 是**那一句本身的问题** —— 「Hello, hello, hello, hello.」是 2.64s 的
重复短语,音素覆盖极窄,声纹本就抽不稳;不是「离线标定在真实条件下失效」。

**撤回上一节的两句话:**「真实条件下同人与异人的分布是重叠的」和「P6d 的绝对数字不能当真值上限用」
—— 一个离群样本不足以支撑这种结论,当时不该那么写。目前证据是:正常长度、正常语气的句子,
线上线下对得上;短促重复的句子会显著偏低(这本就是 minDuration 存在的理由)。

θ_me = 0.60 维持不变,**且现在有正面证据支持**(0.79/0.80 都有充足余量)。

### 3. 麦克风"已授权却采不到声音"—— 仍是 ad-hoc 签名

期间出现过一次:`AVCaptureDevice.authorizationStatus` 报 `3(已授权)`,但麦克风轨
**一条终句都没有**(同一场系统音轨有 11 条)。重新打包后再启动,`requestAll` 报的是
`notDetermined`、系统重新弹框,授权后麦克风立刻正常(帧峰值 7495/32768)。

推断(未逐层验证):ad-hoc 重签后 TCC 记录对 bundle id 仍报「已授权」,但实际采集拿不到数据,
直到重新走一次授权。**根因与「每次启动反复弹权限框」是同一个**:designated requirement
就是二进制 CDHash,重打包即失效。修法见 `scripts/build-app.sh` 注释(造一个固定的自签名证书)。
